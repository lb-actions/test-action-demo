#!/usr/bin/env python3
"""
OpenLibing OBS Bucket 元数据和测试结果上传脚本

=== 使用方法 ===

【Shell 命令行调用】
# 方式1：冒烟测试场景（流水线参数必填，归档为 testcase-metadata/{pipeline_id}/{pipeline_run_id}/{job_id}/{filename}）
python upload_to_openlibing.py \
    --files /path/to/metadata.xml /path/to/test-results_1.xml \
    --pipeline-id "pipeline-123" \
    --pipeline-run-id "run-456" \
    --job-id "job-789" \
    --openlibing-secret '{"apig_code": "xxx", "apig_key": "xxx", "apig_secret": "xxx"}'

# 方式3：label + archive-path（归档为 /{label}/{archive_path}/{filename}）
python upload_to_openlibing.py \
...
# 方式5：通过环境变量传递 secret（无需 --openlibing-secret 参数）
export OPENLIBING_SECRET='{"apig_code": "xxx", "apig_key": "xxx", "apig_secret": "xxx"}'
python upload_to_openlibing.py \
...
file_paths = [
    Path("/path/to/metadata.xml"),
    Path("/path/to/test-results_1.xml")
]
...
# 自定义归档路径（使用 label 和 archive_path）
response = upload_data_to_openlibing(
    file_paths=file_paths,
    openlibing_secret=openlibing_secret,
    upload_config={
        "label": "performance",
        "archive_path": "20260612/143025"
    }
)
print(f"状态码: {response.status_code}, 响应: {response.text}")
...
response = upload_data_to_openlibing(
    ...
    upload_config={
        ...
        "pipeline_run_id": "run-456",
        "job_id": "job-789",
        "url": "https://custom-url.com/upload",
        "label": "precision",
        "archive_path": "project-a/run-001"
    }
)
print(f"状态码: {response.status_code}, 响应: {response.text}")
...
必填参数:
    --files: 要上传的文件路径列表（支持任意文件类型，空格分隔）
    --openlibing-secret: JSON 字符串，包含 apig_code, apig_key, apig_secret
                         支持标准 JSON 格式 {"key": "value"} 和简化格式 {key: value}
                         若未传入，则从环境变量 OPENLIBING_SECRET 读取
                         两者均未提供时报错退出
    
可选参数:
    --pipeline-id: 流水线 ID
    --pipeline-run-id: 流水线运行 ID
    --job-id: 任务 ID
    --url: 自定义上传 URL（默认为 OpenLibing 生产环境 URL）
    --label: 标签，如 "precision(精准测试),performance(性能测试),simulation(仿真测试)"，
                      可单独传入（归档为 /{label}/{filename}），也可与 --archive-path 同时传入
    --archive-path: 自定义归档路径，需与 --label 同时传入（归档为 /{label}/{archive_path}/{filename}）
                    优先级高于 --pipeline-id/--pipeline-run-id/--job-id
                    至少需要提供 --label 或 --pipeline-id/--pipeline-run-id/--job-id 之一
    --help: 显示帮助信息并退出
codeatrs流水线参数配置请参考：https://wiki.huawei.com/domains/3627/wiki/8/WIKI2026031710435645?title=_fc6d722c
"""
...
def main():
    ...
    parser.add_argument(
        "--pipeline-run-id", default="", help="Pipeline run ID (optional)"
    )
    ...
    try:
        ...
        has_pipeline_params = bool(
            args.pipeline_id or args.pipeline_run_id or args.job_id
        )
        ...
        if not has_label and not has_pipeline_params:
            parser.error(
                "must provide either --label or --pipeline-id/--pipeline-run-id/--job-id"
            )

        secret_raw = args.openlibing_secret or os.environ.get("OPENLIBING_SECRET", "")
        if not secret_raw:
            logger.error(
                "openlibing-secret not provided via --openlibing-secret or OPENLIBING_SECRET env var"
            )
            exit(1)
